...
/************************** PRIVATE VARIABLES *************************/
uint8_t menu1[] = "Hello Sami \n\r";
uint8_t menu2[] =
"UART1 Full Modem \n\r\t "
"MCU LPC1768 - ARM Cortex-M3 \n\r\t "
"UART1 - 9600bps \n\r";
uint8_t menu3[] = "UART Task terminated!\n";
...
void print_menu(void)
{
	...
	while(tmp) {
		...
		//tmp2 = UARTSend(LPC_UART1, pDat, tmp);
		tmp2 = UART_Send(LPC_UART1, pDat, tmp,BLOCKING);
		pDat += tmp2;
		tmp -= tmp2;
	}
    ...
    while (exitflag == RESET)
    {
        ...
        while (idx < len)
        {
            if (buffer[idx] == 27)
            {
            	...
            	UART_Send((LPC_UART_TypeDef *)LPC_UART1, menu3, sizeof(menu3),BLOCKING);
                exitflag = SET;
            }
            ...
            else
            {
            	...
            	UART_Send((LPC_UART_TypeDef *)LPC_UART1, &buffer[idx], 1,BLOCKING);
            }
            idx++;
        }
        vTaskDelay(5);
